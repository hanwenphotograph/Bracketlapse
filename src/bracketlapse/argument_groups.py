...
def add_fuse_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--pattern", default="*.jp*g", help="Input glob pattern. Default: *.jp*g"
    )
...
def add_merge_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--merge-subdirs", action="store_true", help="Merge image subdirectories."
    )
    parser.add_argument(
        "--merge-dirs",
        nargs="+",
...
def add_fusion_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--group-size", type=int, default=3, help="Bracket group size. Default: 3"
    )
    parser.add_argument(
        "--sort",
        choices=("name", "time"),
    ...
    parser.add_argument(
        "--align", action="store_true", help="Align groups before fusing."
    )
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing outputs."
    )
    parser.add_argument(
        "--start-number", type=int, default=1, help="Starting output number."
    )
    parser.add_argument("--limit", type=int, help="Process at most this many groups.")
    parser.add_argument(
        "--ext", choices=("jpg", "tif"), default="jpg", help="HDR extension."
    )
    parser.add_argument(
        "--no-video",
        action="store_true",
...
def add_video_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--overwrite", action="store_true", help="Overwrite the output video."
    )
...
def add_video_encoding_arguments(
    parser: argparse.ArgumentParser, *, automatic: bool
) -> None:
    parser.add_argument(
        "--fps",
        type=float,
    ...
    parser.add_argument(
        "--crf", type=int, default=18, help="x265 CRF quality. Default: 18"
    )
